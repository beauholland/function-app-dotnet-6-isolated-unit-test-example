...
public class MockFunctionDefinition : FunctionDefinition
{
    ...
    public static FunctionDefinition Generate(int inputBindingCount = 0, int outputBindingCount = 0, params Type[] paramTypes)
    {
        ...
        inputs.Add($"triggerName", new MockBindingMetadata("TestTrigger", "TestTrigger", BindingDirection.In));

        for (var i = 0; i < inputBindingCount; i++)
        {
            inputs.Add($"inputName{i}", new MockBindingMetadata($"TestInput{i}", $"TestInput{i}", BindingDirection.In));
        }

        for (var i = 0; i < outputBindingCount; i++)
        {
            outputs.Add($"outputName{i}", new MockBindingMetadata($"TestOutput{i}", $"TestOutput{i}", BindingDirection.Out));
        }
